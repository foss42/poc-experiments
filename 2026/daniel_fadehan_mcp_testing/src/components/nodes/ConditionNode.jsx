...
import { ConditionIcon } from './shared/NodeIcons';
...
export const ConditionNode = memo(({ id, data, selected }) => {
  ...
  return (
    <NodeWrapper nodeId={id} nodeType="condition" selected={selected} executionStatus={executionStatus}>
      {/* Node Shape */}
      <div className="relative">
        ...
        <div className={`w-16 h-16 rounded-2xl bg-white border-2 flex items-center justify-center shadow-sm transition-all ${getNodeBorderClass(selected, executionStatus)}`}>
          <div className={`w-10 h-10 rounded-[10px] ${meta.bgColor} flex items-center justify-center`}>
            <ConditionIcon className="w-5 h-5 text-white" />
          </div>
        </div>
